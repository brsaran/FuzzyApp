...
/*  $Id: apam.c 928 2012-05-02 06:12:08Z wrp $ */
/* $Revision: 928 $  */
...
standard_pam(char *smstr, struct pstruct *ppst, int del_set, int gap_set) {
  ...
  for (std_pam_p = std_pams; std_pam_p->abbrev[0]; std_pam_p++ ) {
    if (strcmp(smstr,std_pam_p->abbrev)==0) {	/* if the name matches */
      pam = std_pam_p->pam;	/* set the pam global pointer to the triangular array */
      strncpy(ppst->pam_name,std_pam_p->name,MAX_FN);	/* update the name of the matrix */
      ppst->pam_name[MAX_FN-1]='\0';
      if (ppst->pam_ms) {
	strncat(ppst->pam_name,"-MS",MAX_FN-strlen(ppst->pam_name)-1);
      }
